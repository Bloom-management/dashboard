...
export function PushSettings(){
 ...
 async function run(fn:()=>Promise<void>){if(lock.current)return;lock.current=true;setBusy(true);setError('');setMessage('');try{await fn();}catch(e){setError(e instanceof Error?e.message:'Please try again.');}finally{setBusy(false);lock.current=false;}}
 ...
 {open&&<Modal title="Job notification settings" className="owner-create-dialog bloom-push-dialog" onClose={()=>setOpen(false)}><h2>Job notifications</h2><p role="status">{state}</p>
 ...
 {!status?.configured?<p>Notifications are not available for this account yet. Your admin is completing setup.</p>:<button className="bloom-button" disabled={busy||!status.appId||!sdkReady} onClick={()=>void run(async()=>{
 ...
 })}>{busy?'Working…':status.verified?'Verify this device again':'Enable notifications'}</button>}</>}
