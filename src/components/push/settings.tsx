...
export function PushSettings(){
 ...
 useEffect(()=>{const show=()=>setOpen(true);window.addEventListener('bloom:notification-settings',show);return()=>window.removeEventListener('bloom:notification-settings',show);},[]);
 ...
 return <><section className="bloom-push-card" hidden={state==='Enabled on this device'}><div><h2>Enable job notifications</h2><p>Hear about available cleanings in your approved city and get reminders for jobs you claim.</p><p role="status">{state}{status?.pending?' · Tap the verification notification to finish.':''}</p></div><button className="bloom-button secondary" onClick={()=>setOpen(true)}>Notification settings</button></section>
 ...
 </Modal>}{!open&&!!error&&<p role="alert">{error}</p>}</>;
}
