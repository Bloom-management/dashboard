...
export function BookingCalendar({ month, today = todayIn('America/Detroit'), children, connected = false, onDaySelect, dayLabel = "jobs", dayMinHeight }: { month: string; today?: string; connected?: boolean; dayLabel?: string; dayMinHeight?: (date:string)=>number; onDaySelect?: (date:string)=>void; children: (date: string) => ReactNode }) {
  ...
  function BookingDay({ day, modifiers, ...props }: DayProps) {
    ...
    return <td {...props} data-current-date={value === today || undefined}><div style={dayMinHeight?{minHeight:dayMinHeight(value)}:undefined} className={`${styles.bookingCell} ${onDaySelect?styles.selectableCell:''}`}>
      {onDaySelect&&!modifiers.outside&&<button type="button" className={styles.dayExpand} data-calendar-date={value} aria-label={`View ${dayLabel} for ${value}`} onClick={()=>onDaySelect(value)}/>}
