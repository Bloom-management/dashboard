...
import {OutlineButton} from '../bloom/outline-button';
...
export default function JobMap({points,selected,onSelect,locate=true}:{points:MapPoint[];selected:string|null;onSelect:(id:string)=>void;locate?:boolean}){
 ...
 useEffect(()=>{if(!ready||!map.current)return;const instance=map.current;let cancelled=false;void import('mapbox-gl').then(({default:gl})=>{
  ...
  const bounds=new gl.LngLatBounds();for(const group of groupPoints(points)){
   const first=group[0];bounds.extend([first.longitude,first.latitude]);const button=document.createElement('button');button.type='button';button.className='bloom-map-marker';const logo=document.createElement('img');logo.src='/icon.svg';logo.alt='';logo.width=26;logo.height=26;button.appendChild(logo);if(group.length>1){const count=document.createElement('span');count.className='bloom-map-count';count.textContent=String(group.length);button.appendChild(count);}button.setAttribute('aria-label',group.map(p=>p.label).join(', '));const active=group.some(p=>p.id===selection.current);button.classList.toggle('selected',active);button.setAttribute('aria-pressed',String(active));
 ...
 return <section className="bloom-job-map" aria-label="Map of selected-day cleanings"><div ref={container} className="bloom-map-canvas"/>{!!error&&<p role="status">{error}</p>}{locate&&<OutlineButton className="bloom-map-locate" disabled={!ready||locating} onClick={()=>void useLocation()}>{locating?'Finding location…':'Use my location'}</OutlineButton>}{!!locationMessage&&<p role="status">{locationMessage}</p>}</section>;
}
