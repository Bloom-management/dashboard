...
export function PendingInvitations({revision}:{revision:number}){
 ...
 return <section aria-label="Pending invitations"><h3>Pending invitations</h3>{data.loading?<Loading/>:data.error?<ErrorNotice error={data.error} retry={data.reload}/>:data.data?.length?<PeoplePage items={data.data} label="Pending invitations">{items=><div className="admin-people-table-scroll"><table className="admin-people-table"><thead><tr><th>Email</th><th>Role</th><th>Invitation</th></tr></thead><tbody>{items.map(invite=><tr key={invite.id}><td>{invite.email} <span className="bloom-pending-label">Pending</span></td><td>{invite.role??'Not specified'}</td><td>{invite.role?<ResendInvite email={invite.email} invitationId={invite.id} path="/admin/invitations" onSent={()=>{}}/>:<span>Manage this invitation in Clerk.</span>}</td></tr>)}</tbody></table></div>}</PeoplePage>:<p>No pending invitations.</p>}</section>;
}
