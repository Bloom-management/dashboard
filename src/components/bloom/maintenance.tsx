...
export function MaintenanceGrid({reports=[],showHeading=true}:{reports?:MaintenanceReport[];showHeading?:boolean}){
 return <>{showHeading&&<h3>Maintenance</h3>}<p>Last reported condition · Unreported items have not been checked.</p><div className="maintenance-grid owner-supply-grid">{maintenanceItems.map(item=>{const report=reports.find(r=>r.category===item.category);return <article key={item.category} className="maintenance-card owner-supply-card" data-level={report?.status==='attention'?'low':'unknown'}><span className="bloom-maintenance-icon-stage"><MaintenanceIcon path={item.path}/></span><strong>{item.label}</strong><span>{report?maintenanceStatuses[report.status]:'Not reported'}</span>{report?.notes&&<p>{report.notes}</p>}{report?.reportedAt&&<small>Last reported {new Date(report.reportedAt).toLocaleString()}</small>}</article>;})}</div></>;
}
