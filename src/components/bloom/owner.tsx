...
function OwnerContent({ user, integration }: { user: SessionUser; integration: BloomIntegration }) {
  ...
  const legendItems=freshness.data?.filter(item=>visibleProperties.some(property=>property.id===item.propertyId))??[];
  const legendPages=Math.max(1,Math.ceil(legendItems.length/6));
  ...
  return <><header className="topbar bloom-compact-header"><Brand role="owner" /><div className="topbar-right"><HubSelector user={user} view="owner"/><nav className="view-toggle owner-desktop-views" aria-label="Owner views">{(['calendar','performance','listings','people'] as const).map(view=><button type="button" key={view} className={`vt-btn${tab===view?' active':''}`} aria-current={tab===view?'page':undefined} onClick={()=>{setTab(view);setSelected(null);}}>{view[0].toUpperCase()+view.slice(1)}</button>)}</nav><nav className="owner-mobile-views" aria-label="Owner views"><DropdownMenu><DropdownMenuTrigger className="bloom-hub-trigger" aria-label={`Owner view: ${tab[0].toUpperCase()+tab.slice(1)}`}>{tab[0].toUpperCase()+tab.slice(1)}<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="m4 6 4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuRadioGroup value={tab} onValueChange={value=>{if(value==='calendar'||value==='performance'||value==='listings'||value==='people'){setTab(value);setSelected(null);}}} aria-label="Owner views">{(['calendar','performance','listings','people'] as const).map(view=><DropdownMenuRadioItem key={view} value={view} label={view[0].toUpperCase()+view.slice(1)}>{view[0].toUpperCase()+view.slice(1)}</DropdownMenuRadioItem>)}</DropdownMenuRadioGroup></DropdownMenuContent></DropdownMenu></nav><span className="mobile-notification-space" aria-hidden="true"/><div className="bloom-header-account">{integration.accountControl?.(user) ?? <Account user={user} />}</div></div></header><div className="body"><main className={`main owner-main${tab==='listings'?' owner-listings-main':''}`} id="bloom-main"><div className="main-inner"><div className="loc-header"><div><h1 className="loc-name">{user.role==='admin'?'All properties':'Your properties'}</h1><p className="loc-meta">{user.role==='admin'?'Admin view · ':''}Stays, performance and supplies · Dates are local to each property</p></div><div className="owner-header-actions"><button className="bloom-button secondary" onClick={changed}>Refresh</button>{tab==='listings'&&<OwnerAddListingButton role={user.role} onClick={()=>setCreateOpen(true)}/>}</div></div>
