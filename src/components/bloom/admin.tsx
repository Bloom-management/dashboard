...
function PropertyForm({ integration, created }: { integration: BloomIntegration; created: (id:string) => void }) {
  ...
  async function save(event:FormEvent){
    ...
    await mutation.run(input,async key=>{const result=await integration.admin!.createProperty(input,key);created(result.id);});
  }
  return <form className="bloom-admin-card bloom-form" onSubmit={save} aria-busy={mutation.busy}><h2>Add Property</h2>
    ...
    <label>Initial solo rate (USD)<input type="number" required min="0.02" step="0.02" value={rate} onChange={e=>setRate(e.target.value)}/><small>Shared rate: {rateCents === null ? 'Enter a valid solo rate' : money(rateCents/2)} per cleaner. Rates are set per unit; later rate editing remains deferred.</small></label>
    {validation && <p className="bloom-notice" role="alert">{validation}</p>}<button className="bloom-button" disabled={mutation.busy||cities.loading||!!cities.error||!cities.data?.some(c=>c.active)||!propertyOwnership(draft.isBloomOwned,ownerMode,draft.ownerIds,ownerEmail)}>{mutation.busy?'Saving…':'Save property'}</button><Result mutation={mutation}/>
  </form>;
}
...
function AdminJob({ job, user, integration, close, reload }: { job: CleanerJob; user: SessionUser; integration: BloomIntegration; close: () => void; reload: () => void }) {
  ...
  return <article className={styles.detail}><div className={styles.detailToolbar}><h2>Manage job and view photos</h2></div><div className="detail-hero"><h2>{job.propertyName}</h2><p>{formatDate(job.checkoutDate)} · {formatTime(job.startAt, job.timezone)}–{formatTime(job.endAt, job.timezone)} · {job.timezone}</p><p>{job.status} · {job.activeCleanerCount}/2 assigned</p></div><div className="detail-body"><Changes changes={job.changes} /><Photos job={job} user={user} integration={integration} />{job.status==='completed'&&<CompletionReport jobId={job.id}/>}{job.status === 'open' && <div className="bloom-form"><label>Reason for change<textarea required value={reason} onChange={event => setReason(event.target.value)} /></label>{job.reviewRequired && <><p className="bloom-notice">Resolve the booking change before claiming or completion can resume.</p><DatePicker label="New checkout date" value={checkoutDate} onChange={setCheckoutDate} /><div className="bloom-actions"><button className="bloom-button" disabled={mutation.busy || !reason.trim()} onClick={() => change('keep')}>Keep schedule</button><button className="bloom-button" disabled={mutation.busy || !reason.trim() || !checkoutDate} onClick={() => change('reschedule')}>Reschedule</button><button className="bloom-button secondary" disabled={mutation.busy || !reason.trim()} onClick={() => change('resolve-cancel')}>Resolve by cancelling</button></div></>}
...
function Jobs({ user, integration }: { user: SessionUser; integration: BloomIntegration }) {
  ...
  return <><div className={styles.filters}>{properties.loading ? <Loading /> : properties.error ? <ErrorNotice error={properties.error} retry={properties.reload} /> : properties.data ? <LocationDropdown label="Filter jobs by property" placeholder="All properties" searchLabel="Search properties" locations={[{ id: '', name: 'All properties' }, ...properties.data]} value={propertyFilter} onValueChange={value => { setPropertyFilter(value); setSelectedId(null); }} /> : <p>Property filtering is unavailable.</p>}</div><div className={styles.split}>
    ...
    <section className={styles.calendar} aria-label="Cleaning calendar">
      <MonthHead month={month} onMonth={value => { setSelectedId(null); setMonth(value); }} />
      ...
      {jobs.loading ? <Loading /> : jobs.error ? <ErrorNotice error={jobs.error} retry={jobs.reload} /> : <>
        <BookingCalendar month={month}>{date => visibleJobs?.filter(job => job.checkoutDate === date).map(job => <button type="button" key={job.id} data-cleaning-job={job.id} className={styles.cleaning} data-selected={selectedId === job.id} aria-pressed={selectedId === job.id}
          ...
          onClick={() => { setSelectedId(job.id); requestAnimationFrame(() => panel.current?.focus({ preventScroll: window.matchMedia('(min-width: 1000px)').matches })); }}>
...
function AdminContent({ user, integration, initialPropertyId }: { user: SessionUser; integration: BloomIntegration; initialPropertyId?:string }) {
  ...
  return <><header className="topbar"><Brand role="admin" /><div className="topbar-right"><HubSelector user={user} view="admin"/>{integration.accountControl?.(user) ?? <Account user={user} />}</div></header><main className={`main${view==='properties'?' admin-properties-main':''}`} id="bloom-main"><div className={`main-inner ${(view === 'jobs' || view === 'properties') ? styles.jobsPage : ''}`}><nav className="bloom-admin-tabs" aria-label="Admin tools">{['jobs', 'properties', 'users', 'cities', 'city requests'].map(item => <button key={item} className={`city-tab${item === view ? ' active' : ''}`} aria-pressed={item === view} onClick={() => router.push(`/admin?${new URLSearchParams({view:item})}`)}>{item==='users'?'People':item}</button>)}</nav><h1 className="loc-name">{view==='users'?'People':view.slice(0, 1).toUpperCase() + view.slice(1)}</h1>
    ...
    {view === 'users' && (integration.admin ? <><InvitePerson/><Paginated loader={integration.admin.users} render={(users, reload) => <div className="admin-people-table-scroll"><table className="admin-people-table"><thead><tr><th scope="col">Name</th><th scope="col">Email</th><th scope="col">Role</th></tr></thead><tbody>{users.map(item => <UserRow key={item.id} user={item} currentUser={user} reload={reload} />)}</tbody></table>{!users.length&&<p>No people found.</p>}</div>} /></> : unavailable)}
