...
export function completionInput(version: number, supplyIds: string[], answers: Record<string, SupplyLevel>, notes: string, maintenance?:Partial<Record<MaintenanceCategory,MaintenanceAnswer>>): CompletionInput | null {
  if (notes.length>1500 || supplyIds.some(id=>!Object.hasOwn(supplyLevels,answers[id]))) return null;
  ...
  return {...(maintenance?{maintenance:maintenanceCategories.map(category=>maintenance[category]??{category,status:'not_applicable' as const,notes:''})}:{}),configVersion:version,answers:supplyIds.map(supplyId=>({supplyId,level:answers[supplyId]})),notes};
}
