...
function CleanerContent({ user, refreshUser, integration }: { user: SessionUser; refreshUser: () => void; integration: BloomIntegration }) {
  ...
  return <><header className="topbar bloom-compact-header"><Brand role="cleaner" />{user.role === 'admin' ? <div className="bloom-city"><LocationDropdown label="City filter" value={cityId ?? ''} onValueChange={value => { setAdminCity(value); setProperty(''); setSelected(null); }} locations={cities.data ?? []} disabled={cities.loading || !!cities.error} />{cities.loading && <Loading />}{!!cities.error && <ErrorNotice error={cities.error} retry={cities.reload} />}</div> : <CityPicker user={user} refresh={refreshUser} integration={integration} />}<div className="topbar-right"><HubSelector user={user} view="cleaner" /><div className="view-toggle">{(['calendar', 'upcoming'] as const).map(tab => <button key={tab} className={`vt-btn${view === tab ? ' active' : ''}`} aria-label={tab === 'calendar' ? 'Available-job calendar' : 'My upcoming jobs'} aria-pressed={view === tab} onClick={() => setView(tab)}><Icon name={tab === 'calendar' ? 'grid' : 'list'} /><span>{navigationLabel(tab)}</span></button>)}</div><span className="mobile-notification-space" aria-hidden="true"/><div className="bloom-header-account">{integration.accountControl?.(user) ?? <Account user={user} />}</div></div></header><div className="body"><PropertyRail properties={properties} selected={property} onSelect={setProperty} /><main className="main" id="bloom-main"><div className="main-inner">
