...
import { SupplySetupGrid, CalendarSetupCard } from './supply-setup-grid';
...
export function OwnerCreateListing({open,listings,onClose,onCreated,onChanged}:{open:boolean;listings:OwnerListing[];onClose:()=>void;onCreated:(listing:OwnerListing)=>void;onChanged:()=>void}) {
 ...
 const [calendars,setCalendars]=useState<{id:string;provider:'airbnb'|'vrbo';url:string}[]>([]);
 ...
 function close(){if(busy||locked.current||calendarLock.current)return;onClose();if(created){setCreated(null);setConnect(false);setDraft(initial);setSetupStep(0);setCalendars([]);setCalendarResults({});calendarAttempts.current=[];setSupplies([]);setSuppliesSaved(false);setSupplyError(undefined);supplyAttempt.current=null;setError(undefined);setValidation('');}}
 async function save(event:React.FormEvent){
  ...
  try{const result=await request<OwnerListingCreateResult>('/owner/listings',{body:receipt.current.input,key:receipt.current.key});const input=receipt.current.input;receipt.current=null;setPending(false);setCreated(result.listing);onCreated(result.listing);
...
})}</div></fieldset><div hidden={setupStep!==1}><SupplySetupGrid supplies={supplies} onChange={setSupplies} optional/></div><fieldset hidden={setupStep!==2}><legend>Calendars (optional)</legend><p>Add one or more private iCal export links for this listing. Use calendar export links, not public listing URLs. Airbnb and Vrbo are supported. You can add them later in Settings.</p><div className="owner-create-supply-grid">{calendars.map((feed,index)=><CalendarSetupCard key={feed.id} title={`Calendar ${index+1}`} provider={feed.provider} url={feed.url} onProvider={provider=>setCalendars(current=>current.map(c=>c.id===feed.id?{...c,provider}:c))} onUrl={url=>setCalendars(current=>current.map(c=>c.id===feed.id?{...c,url}:c))} remove={()=>setCalendars(current=>current.filter(c=>c.id!==feed.id))}/>)}</div><button type="button" className="bloom-button secondary" onClick={()=>setCalendars(current=>[...current,{id:crypto.randomUUID(),provider:'airbnb',url:''}])}>+ Add calendar</button></fieldset></fieldset>{cities.loading?<Loading/>:cities.error?<ErrorNotice error={cities.error} retry={cities.reload}/>:!cities.data?.some(city=>city.active)&&<p className="bloom-notice">No supported cities are available. Contact your admin.</p>}{validation&&<p role="alert">{validation}</p>}{!!error&&<ErrorNotice error={error}/>} {pending&&!busy&&<p>Your original details and request are kept for a safe retry. Retry this same listing to check whether it was saved.</p>}<button className="bloom-button" disabled={busy||(!pending&&(cities.loading||!!cities.error||!cityId))}>{busy?'Saving…':pending?'Retry saving this listing':'Save listing'}</button></form></>}</div></Modal>;
}
