...
export function LocationDropdown({ locations, value, onValueChange, label, placeholder = 'Choose city', disabled = false, searchLabel = 'Search locations' }: {
  ...
  return <DropdownMenu onOpenChange={open => { if (open) { setSearch(''); setContainer(trigger.current?.closest('dialog')??null); } }} onOpenChangeComplete={open => { if (open) input.current?.focus(); }}>
    <DropdownMenuTrigger ref={trigger} className={styles.trigger} disabled={disabled} aria-label={`${label}: ${selected?.name ?? placeholder}`}>
