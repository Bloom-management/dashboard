...
class OwnerHub(unittest.TestCase):
 ...
 def test_owner_sources_setup_binding_revocation_and_existing_admin_gate(self):
  ...
  self.assertIn('CONFLICT',self.owner_rpc('add_source',p_input={**self.input,'fingerprint':'e'*64},p_key='different-export').stderr)
  sql(f"update calendar_sources set enabled=false where id='{self.sid}';")
  self.assertIn('CONFLICT',self.owner_rpc('add_source',p_input={**self.input,'fingerprint':'f'*64},p_key='disabled-other-export').stderr)
  ...
  self.assertIn('VALIDATION_ERROR',self.owner_rpc('add_source',p_input={**self.input,'provider':'vrbo'},p_key='vrbo').stderr)
  ...
  self.assertIn('NOT_FOUND',self.owner_rpc('begin_sync',p_source=vrbo,p_key='vrbo-sync').stderr)
